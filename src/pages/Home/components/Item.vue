<template>
  <div
    :class="[
      'item',
      (deleteTodoLoading || deleteArchiveLoading || isTodoDeleted || isArchiveDeleted) &&
        'deleting',
  ...
  >
    ...
    <div class="right">
      ...
      <template v-else>
        <span class="back" title="回档" @click="backArchive(item!!.id)"> 回 </span>
        <span class="delete" title="删除" @click="deleteArchive(item!!.id)"> 删 </span>
      </template>
    </div>
    <div
      class="deleting-mask"
      v-if="deleteTodoLoading || deleteArchiveLoading || isTodoDeleted || isArchiveDeleted"
    >
...
type Emit = (
  event: 'onTodoUpdated' | 'onTodoDeleted' | 'onArchiveDeleted' | 'onBackArchive',
  ...args: unknown[]
) => void
...
const useTodoItem = (props: Props, { emit }: { emit: Emit }) => {
  ...
  const [updateTodo, { loading: updateTodoLoading }] = useMutation(updateTodoApi)
  const [deleteTodo, { loading: deleteTodoLoading }] = useMutation(deleteTodoApi, {
    onCompleted: () => {
...
const useArchive = (props: Props, { emit }: { emit: Emit }) => {
  ...
  const [backArchive, { loading: backArchiveLoading }] = useMutation(backArchiveApi, {
    onCompleted: () => {
      isBacked.value = true
      emit('onBackArchive')
    }
  })
  const [deleteArchive, { loading: deleteArchiveLoading }] = useMutation(deleteArchiveApi, {
    onCompleted: () => {
...
export default defineComponent({
  ...
  emits: ['onTodoUpdated', 'onTodoDeleted', 'onArchiveDeleted', 'onBackArchive'],
  setup(props, ctx) {
